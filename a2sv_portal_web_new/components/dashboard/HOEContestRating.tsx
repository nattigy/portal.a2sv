...
type Props = {
  contestInfo: HOEContestInfo[];
};

const HOEContestRating = ({ contestInfo }: Props) => {
  return (
    <div>
      <div className="overflow-x-auto relative bg-white border-blue-100 shadow-md sm:rounded-lg border p-4 w-fit ">
        ...
        <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {contestInfo ? (
              contestInfo.map(
                (contestRatingInfo: HOEContestInfo, index: number) => {
                  return (
                    <tr
...
export default HOEContestRating;
