...
import {SeasonTopicFilter, SeasonTopicId, SeasonTopicService} from './season-topic.service';
...
export class SeasonTopicResolver {
    ...
    createSeasonTopic(@Args('createSeasonTopicInput') createSeasonTopicInput: CreateSeasonTopicInput) {
    ...
    @Mutation(() => SeasonTopic)
    updateSeasonTopic(@Args('updateSeasonTopicInput') updateSeasonTopicInput: UpdateSeasonTopicInput) {
        return this.seasonTopicService.update(updateSeasonTopicInput);
    }

    @Mutation(() => SeasonTopic)
    removeSeasonTopic(@Args('seasonTopicId', {type: () => Int}) id: SeasonTopicId) {
