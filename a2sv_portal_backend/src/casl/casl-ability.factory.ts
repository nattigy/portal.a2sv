...
export type Subjects =
  InferSubjects<typeof User | typeof Group | typeof Tag | typeof Contest | typeof Problem | typeof Season | typeof Topic | typeof UserProfile | typeof UserContest | typeof UserContestProblem | typeof UserTopic | typeof SeasonTopic | typeof SeasonTopicProblem | typeof SeasonTopicProblemUser>
  | 'all'
...
export class CaslAbilityFactory {
  createForUser(user: User) {
    const {
      ...
      cannot,
      build
    } = new AbilityBuilder<Ability<[Action, Subjects]>>(createMongoAbility)
    switch (user.role) {
      case RoleEnum.STUDENT:
        can(Action.Update || Action.Read || Action.Create, UserContest || UserTopic || UserContestProblem || UserProfile || SeasonTopicProblemUser)
        break
      ...
      case RoleEnum.ASSISTANT:
        can(Action.Update || Action.Read || Action.Create || Action.Delete, UserContest || UserTopic || UserContestProblem || UserProfile || SeasonTopicProblemUser)
        break
    }
    return build()
  }
